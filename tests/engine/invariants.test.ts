...
describe("assertIntegerCents", () => {
  ...
  it.each([[10.5], [-0.01], [Number.NaN], [Number.POSITIVE_INFINITY]])(
    ...
    (value) => {
      expect(() => assertIntegerCents(value, "amountCents")).toThrowError(
        EngineError,
      );
      expect(() => assertIntegerCents(value, "amountCents")).toThrowError(
...
describe("assertPositiveCents", () => {
  ...
  it("rejects floats as non-integer first", () => {
    expect(() => assertPositiveCents(0.5, "cents")).toThrowError(
      /integer number of cents/,
    );
  });
});
...
describe("calendarMonth", () => {
  ...
  it("uses UTC components for Date objects (household-local dates are built as UTC)", () => {
    expect(calendarMonth(new Date("2026-09-03T00:00:00Z"))).toEqual({
      year: 2026,
      month: 9,
    });
  });
  ...
  it("rejects unparseable date strings", () => {
    ...
    expect(() => calendarMonth("2026-13-01")).toThrowError(
      /out-of-range month/,
    );
  });
});
...
describe("previousCalendarMonth", () => {
  it.each([
    ...
    "rolls %i-%i back to %i-%i",
    (year, month, expectedYear, expectedMonth) => {
      expect(previousCalendarMonth(year, month)).toEqual({
        year: expectedYear,
        month: expectedMonth,
      });
    },
